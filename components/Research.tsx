"use client";

import { useTranslation } from "@/lib/i18n";
import { Section, Reveal, SpecRow } from "./Section";

export function Research() {
  const { t } = useTranslation();

  return (
    <Section
      id="research"
      index="02"
      title={t.research.title}
      kicker="Optical Networks"
    >
      <Reveal>
        {/* The research theme is the loudest line on the page after the name. */}
        <p className="jp whitespace-pre-line font-display text-[clamp(1.7rem,4.2vw,3rem)] leading-[1.35]">
          {t.research.theme}
        </p>
      </Reveal>

      <Reveal delay={0.1}>
        <div className="mt-10 w-full">
          <div className="waveguide" />
        </div>
      </Reveal>

      <div className="mt-12 grid gap-12 lg:grid-cols-12">
        <Reveal delay={0.15} className="lg:col-span-7">
          <p className="jp max-w-2xl text-sm leading-loose text-dim md:text-base">
            {t.research.summary}
          </p>

          <h3 className="mt-12 font-mono text-[10px] uppercase tracking-[0.24em] text-dim">
            {t.research.outputsLabel}
          </h3>
          <ol className="mt-5">
            {t.research.outputs.map((o, i) => (
              <li key={o.title} className="flex gap-5 border-t border-rule py-6">
                <span className="shrink-0 font-mono text-[11px] text-accent-text">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <div className="min-w-0">
                  <span className="inline-block border border-rule px-2 py-0.5 font-mono text-[9px] tracking-[0.18em] text-dim">
                    {o.kind}
                  </span>
                  <p className="jp mt-3 font-display text-base leading-relaxed md:text-lg">
                    {o.title}
                  </p>
                  <p className="mt-2 text-xs leading-relaxed text-dim">
                    {o.authors}
                  </p>
                  <p className="mt-1 font-mono text-[10px] leading-relaxed tracking-[0.06em] text-dim">
                    {o.venue}
                  </p>
                </div>
              </li>
            ))}
          </ol>
        </Reveal>

        <Reveal delay={0.2} className="lg:col-span-5">
          <div className="space-y-3 border-t border-rule pt-6">
            {t.research.meta.map((item) => (
              <SpecRow key={item.label} item={item} />
            ))}
          </div>
        </Reveal>
      </div>
    </Section>
  );
}
